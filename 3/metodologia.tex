...
En la Figura \ref{3:fig4} se detalla el tamaño del conjunto de datos total al corte del periodo de captura de información Julio 2019, aproximadamente más de 212 mil proyectos de todas las categorías y 37 columnas de variables.
...
Para seleccionar solamente proyectos de la categoría Tecnología, que representaba en promedio el 10\% de cada dataset, se aplicó la lógica del pseudocódigo de la Figura CITAR.

A cada conjunto de datos generado se filtran los que pertenecen a la categoría \textit{\textbf{Technology}}. Al no contar con la información de la columna \textit{main\_category}, este proceso se logró utilizando la variable \textit{source\_url} seleccionando aquellos registros que contengan la cadena de caracteres “\textbf{https://www.kickstarter.com/discover/categories/technology}”. En la Figura \ref{3:fig5} se aprecia el resultado del conjunto filtrado por categoría Techonlogy al corte de Julio 2019.
...
La siguiente fase del pre-procesamiento fue la selección de variables y generación de algunas faltantes. Para ello, se usó el software Alteryx Designer, el cual permite desarrollar flujos de trabajo para preparar, unir y analizar volúmenes de datos complejos de distintas fuentes. Se ejecutó el flujograma representado de forma resumida en la Figura \ref{3:fig6}.

\begin{figure}[h]
	\begin{center}
		\includegraphics[width=1\textwidth]{3/figures/flujograma_metadata.png}
		\caption[Flujograma resumido del pre-procesamiento de Metainformación]{Flujograma resumido del pre-procesamiento de Metainformación.\\
			Fuente: Elaboración propia.}
		\label{3:fig6}
	\end{center}
\end{figure}

El flujo comienza uniendo los 45 archivos separados por coma (.csv) capturados mensualmente desde noviembre del 2015 hasta agosto del 2019. Se realizaron 2 uniones internamente ya que, a partir de marzo del 2018, algunas de las variables y valores presentan diferente estructura a la de sus predecesoras. Sin embargo, esto no afectó al proceso al cruzar por el nombre de columna. Las variables escogidas se listan en la Tabla \ref{3:table5}.
...
El análisis exploratorio de los datos fue determinante para la selección final de variables de la Metainformación, gracias al uso de herramientas estadísticas como el análisis de correlaciones para discriminar aquellas que superaron el límite fijado y que desembocó, luego del pre-procesamiento detallado en el subcapítulo 3 del mismo capítulo, en la Tabla \ref{3:table6}.
