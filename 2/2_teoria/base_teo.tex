...
\begin{itemize}
	...
	Mientras que las convoluciones unidimensionales (1d) son muy útiles para series de tiempo y operaciones de NLP por estar conformadas por vectores, donde cada palabra es un vector, aprendiendo así patrones en la dimensión de secuencia \parencite{bk_rao2019nlp_pytorch}. En la Figura \ref{2:fig41} se observa un ejemplo.
	\begin{figure}[h]
		\begin{center}
			\includegraphics[width=0.5\textwidth]{2/figures/1D-convolutional-example.png}
			\caption{Ejemplo de convolución 1d. Fuente: \cite{tec_missinglink_conv1d}}
			\label{2:fig41}
		\end{center}
	\end{figure}
	
	\item \textbf{Redes Neuronales Recurrentes} (\textit{Recurrent Neural Networks}):
